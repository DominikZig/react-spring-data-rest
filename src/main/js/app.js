...
class App extends React.Component {
    ...
    loadFromServer(pageSize) {
        ...
        }).then(employeePromises => { //then() function always needs to return something
            return when.all(employeePromises);
        }).done(employees => { //done function does not return anything, and don't chain anything after it
            this.setState({
                page: this.page,
                employees: employees,
        ...
        client({
            ...
            if (response.status.code === 412) {
                alert('DENIED: Unable to update ' +
                    employee.entity._links.self.href + '. Your copy is stale.'); //Using ETags and Versioning, only one user can update an Employee at a time
            }
        });
    }
...
class EmployeeList extends React.Component {
    ...
    render() {
        ...
        const employees = this.props.employees.map(employee =>
            <Employee key={employee.entity._links.self.href} //this is creating a new component called 'Employee'
                      employee={employee}
